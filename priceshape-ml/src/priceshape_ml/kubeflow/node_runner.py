...
`priceshape_ml/dag.py` pickles each node's callable to object storage and compiles one
...
1. **Every import is inside the function.** KFP ships only the function's source
   ...
   `priceshape_ml/kubeflow/storage.py` — `priceshape_ml/` is not in any image.

2. **The base image must be able to unpickle the callable.** The pickle references
   `core.components...`, so the image needs the package installed. Point
