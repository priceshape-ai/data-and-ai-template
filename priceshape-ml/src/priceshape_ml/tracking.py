...
gathered here — `priceshape_ml/gitgate.py` owns the git introspection. So this module
...
def log_run(
    ...
    Args:
        ...
        results: `{node_name: NodeResultLike}`. Metrics are logged as
...
def collect_metrics(results: dict[str, Any]) -> dict[str, float]:
    """Flatten `{node: NodeResultLike}` into `{"<node>.<metric>": value}`."""
